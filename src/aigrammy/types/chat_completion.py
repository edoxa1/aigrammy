...
class GptChatCompletionRepo:
    ...
    async def ask_text(self, prompt: str, max_tokens=1000) -> GptResponse:
    ...
    async def ask_telegram_image(self,
                                 binary_file: BinaryIO,
                                 max_tokens=500,
                                 prompt: str = "(no additional info was specified)") -> GptResponse:
        """
        ...
        :param binary_file: Binary from `aiogram.bot.download_file(file_id: str)`.
        ...
        :param prompt: prompt which will be sent with given image
